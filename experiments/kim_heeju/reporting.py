"""EDA·통계 검정·9개 요금 모델 결과를 Markdown 보고서로 생성한다."""
...
def _code_list(values: list[str]) -> str:
    """코드 형식의 Markdown 목록을 만든다."""
    return ", ".join(f"`{value}`" for value in values)


def generate_report(results: dict, output_path: Path = BASE_DIR / "report.md") -> Path:
    """실행 결과와 구현 설정을 상세 Markdown 보고서로 저장한다."""
    statistics = results["statistics"]
    test = statistics["ttest"]
    model = results["model"]
    total = model["total_metrics"]
    p_value = "< 1e-300" if test["p_value"] == 0 else f"{test['p_value']:.6g}"
    distance_fare_correlation = statistics["correlation"].loc[
        "trip_distance", "fare_amount"
    ]
    regressor_parameters = (
        f"`n_estimators=250`, `random_state={RANDOM_STATE}`, "
        "`n_jobs=-1`, `verbosity=-1`"
    )
    classifier_parameters = (
        f"`n_estimators=200`, `random_state={RANDOM_STATE}`, "
        "`n_jobs=-1`, `verbosity=-1`"
    )

    upper_limit_rows = "\n".join(
        f"| `{target}` | ${limit:.2f} |" for target, limit in FARE_UPPER_LIMITS.items()
    )
    zero_ratio_rows = "\n".join(
        f"| `{target}` | {ratio:.2%} |" for target, ratio in statistics["zero_ratio"].items()
    )
    regression_rows = "\n".join(
        f"| `{target}` | {metric['test_rows']:,} | {metric['mae']:.3f} | "
        f"{metric['rmse']:.3f} | {metric['r2']:.3f} | {metric['baseline_mae']:.3f} |"
        for target in [*CONTINUOUS_TARGETS, *ZERO_INFLATED_TARGETS]
        for metric in [model["metrics"][target]]
    )
    two_stage_rows = "\n".join(
        f"| `{target}` | {model['metrics'][target]['charge_f1']:.3f} |"
        for target in ZERO_INFLATED_TARGETS
    )
    classification_rows = "\n".join(
        f"| `{target}` | {metric['test_rows']:,} | {metric['accuracy']:.3f} | "
        f"{metric['weighted_f1']:.3f} | {metric['baseline_accuracy']:.3f} | "
        f"${metric['mae']:.3f} |"
        for target in FIXED_FEE_TARGETS
        for metric in [model["metrics"][target]]
    )

    content = f"""# NYC Yellow Taxi 요금 구성요소 예측 보고서
...
- 데이터 출처: {DATA_URL}
- 원본 형식: Parquet
- 정제 후 데이터: {results['cleaning']['pandas_rows']:,}건
- 학습 기간: 5월 1~24일, {model['train_rows']:,}건
- 테스트 기간: 5월 25~31일, {model['test_rows']:,}건
- 공통 알고리즘: LightGBM
...
- `trip_distance`와 `fare_amount`의 상관계수: {distance_fare_correlation:.3f}
...
- 평일: {test['weekday_count']:,}건, 평균 ${test['weekday_mean']:.2f}
- 주말: {test['weekend_count']:,}건, 평균 ${test['weekend_mean']:.2f}
- t 통계량: {test['statistic']:.4f}
- p-value: {p_value}
...
{_code_list(CATEGORICAL_FEATURES)}
...
{_code_list(FIXED_FEE_TARGETS)}는 연속적인 임의 금액이 아니라 몇 가지 정해진 금액이
...
| 모델 | 명시 파라미터 |
|---|---|
| `LGBMRegressor` | {regressor_parameters} |
| `LGBMClassifier` | {classifier_parameters} |

- `n_estimators`는 생성할 boosting tree 수이다.
- `random_state={RANDOM_STATE}`로 재현 가능성을 높였다.
...
- 평가 행: {total['test_rows']:,}건
- MAE: **${total['mae']:.3f}**
- RMSE: **${total['rmse']:.3f}**
- R²: **{total['r2']:.3f}**
- 실제 9개 구성요소 합계와 `total_amount` 자체의 MAE: **${total['component_sum_mae']:.3f}**

최종 기준은 9개 예측값의 합계와 실제 `total_amount`의 MAE다. 현재 모델은
최종 요금을 평균적으로 약 ${total['mae']:.2f} 차이로 예측했다. 실제 9개 요금을
그대로 더해도 `total_amount`와 평균 ${total['component_sum_mae']:.3f}의 차이가 있어, 모델
