...
from experiments.kim_yechan.data_loader import load_taxi_pandas
from experiments.kim_yechan.preprocessing import HOLDOUT_START, prepare_modeling_data
from experiments.kim_yechan.statistical_analysis import run_statistical_analysis
...
AMOUNT_BENCHMARK_PATH = METRICS_DIR / "amount_target_benchmark.csv"
...
def run_amount_target_benchmark(
    train: pd.DataFrame, test: pd.DataFrame
) -> pd.DataFrame:
    """동일한 누수 없는 피처로 요금 구성 항목을 각각 예측한다."""
    results: list[dict[str, float | int | str]] = []
    for target in LEAKAGE_COLUMNS:
        train_valid = train.loc[train[target].notna()]
        test_valid = test.loc[test[target].notna()]
        if train_valid.empty or test_valid.empty:
            raise ValueError(f"{target}의 학습 또는 테스트 정답이 비어 있습니다.")

        pipeline = Pipeline(
            steps=[
                ("preprocessor", build_preprocessor("route_statistics")),
                ("regressor", Ridge(alpha=10.0, solver="lsqr")),
            ]
        )
        started = time.perf_counter()
        pipeline.fit(train_valid[MODEL_FEATURES], train_valid[target])
        fit_seconds = time.perf_counter() - started
        predictions = pipeline.predict(test_valid[MODEL_FEATURES])
        metrics = calculate_metrics(test_valid[target], predictions)
        results.append(
            {
                "target": target,
                "train_rows": len(train_valid),
                "test_rows": len(test_valid),
                "test_mean": float(test_valid[target].mean()),
                "test_nonzero_rate": float(test_valid[target].ne(0).mean()),
                **metrics,
                "fit_seconds": fit_seconds,
            }
        )
...
def create_amount_benchmark_chart(results: pd.DataFrame) -> Path:
    """요금 구성 항목별 R²를 비교하고 평균선을 표시한다."""
    path = FIGURE_DIR / "amount_target_r2_comparison.png"
    average_r2 = float(results["r2"].mean())
    ordered = results.sort_values("r2", ascending=False)
    fig, ax = plt.subplots(figsize=(11, 7))
    sns.barplot(data=ordered, x="r2", y="target", color="#4C78A8", ax=ax)
    ax.axvline(
        average_r2,
        linestyle="--",
        color="#E45756",
        label=f"Mean R² = {average_r2:.4f}",
    )
    ax.set(
        title="Amount Component Prediction — Leakage-Free R²",
        xlabel="R² on temporal holdout",
...
def generate_report(
    ...
    amount_rows = [
        [
            ...
            f"${row.mae:.4f}",
            f"${row.rmse:.4f}",
            f"{row.test_nonzero_rate:.2%}",
            f"{int(row.test_rows):,}",
        ]
        for row in amount_results.itertuples(index=False)
    ]
    amount_r2_lines = "\n".join(
        f"{row.target}: {row.r2:.4f}"
        for row in amount_results.itertuples(index=False)
    )
    average_amount_r2 = float(amount_results["r2"].mean())
...
전체 항목 평균 R2 Score: {average_amount_r2:.4f}
```

{markdown_table(['정답 항목', 'R²', 'MAE', 'RMSE', '비영 비율', '테스트 행'], amount_rows)}
...
def save_results(
    ...
    amount_results.to_csv(AMOUNT_BENCHMARK_PATH, index=False)
    summary = {
        ...
        "amount_target_benchmark": json.loads(
            amount_results.to_json(orient="records")
        ),
        "amount_target_mean_r2": float(amount_results["r2"].mean()),
    }
...
def run_modeling(sample_size: int = DEFAULT_SAMPLE_SIZE) -> dict[str, float]:
    ...
    amount_results = run_amount_target_benchmark(train, test)
    ...
    create_amount_benchmark_chart(amount_results)
    save_results(
        pipeline,
        preprocessing,
        audit,
    ...
    print("\n[항목별 R2 Score 성능]")
    for row in amount_results.itertuples(index=False):
        print(f"{row.target}: {row.r2:.4f}")
    print(f"전체 항목 평균 R2 Score: {amount_results['r2'].mean():.4f}")
    print(f"보고서: {report}")
