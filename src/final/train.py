...
MODEL_CHART_PATH = FIGURE_DIR / "model_evaluation.png"
...
def get_environment_versions() -> dict[str, str]:
    """모델 재현에 필요한 Python과 주요 패키지 버전을 반환한다."""
    packages = (
        "pandas",
        "polars",
        "pyarrow",
...
def load_with_pandas_and_polars() -> tuple[pd.DataFrame, dict[str, Any]]:
    ...
    pandas_memory_bytes = int(pandas_data.memory_usage(deep=True).sum())
    polars_memory_bytes = int(polars_data.estimated_size())
...
def create_visualizations(dataframe: pd.DataFrame) -> dict[str, Any]:
    ...
    interactive.write_html(
        ...
        div_id="hourly-total-amount-chart",
    )
    return {
...
def create_model_evaluation_chart(
    y_true: np.ndarray,
    predictions: np.ndarray,
    fare_bands: list[dict[str, Any]],
) -> None:
    """테스트 예측의 적합도, 잔차, 금액 구간별 오차를 시각화한다."""
    generator = np.random.default_rng(RANDOM_STATE)
    sample_size = min(50_000, len(y_true))
    sample_index = generator.choice(len(y_true), size=sample_size, replace=False)
    sampled_actual = y_true[sample_index]
    sampled_predictions = predictions[sample_index]
    residuals = predictions - y_true
    residual_lower, residual_upper = np.quantile(residuals, [0.01, 0.99])

    figure, axes = plt.subplots(1, 3, figsize=(18, 5))
    visible = (sampled_actual <= 150) & (sampled_predictions <= 150)
    axes[0].scatter(
        sampled_actual[visible],
        sampled_predictions[visible],
        s=7,
        alpha=0.15,
        color="#3366cc",
        edgecolors="none",
    )
    axes[0].plot([0, 150], [0, 150], linestyle="--", color="#d62728")
    axes[0].set_title("Actual vs Predicted (≤ $150)")
    axes[0].set_xlabel("Actual total amount (USD)")
    axes[0].set_ylabel("Predicted total amount (USD)")
    axes[0].set_xlim(0, 150)
    axes[0].set_ylim(0, 150)

    sns.histplot(
        residuals[(residuals >= residual_lower) & (residuals <= residual_upper)],
        bins=80,
        color="#00a878",
        ax=axes[1],
    )
    axes[1].axvline(0, linestyle="--", color="#d62728")
    axes[1].set_title("Residual Distribution (1st–99th percentile)")
    axes[1].set_xlabel("Prediction − actual (USD)")
    axes[1].set_ylabel("Trip count")

    band_names = [result["fare_band"] for result in fare_bands]
    band_mae = [result["mae"] for result in fare_bands]
    sns.barplot(x=band_names, y=band_mae, color="#f4b400", ax=axes[2])
    axes[2].set_title("MAE by Actual Fare Band")
    axes[2].set_xlabel("Actual fare band (USD)")
    axes[2].set_ylabel("MAE (USD)")
    axes[2].tick_params(axis="x", rotation=20)
    for index, value in enumerate(band_mae):
        axes[2].text(index, value, f"{value:.2f}", ha="center", va="bottom")

    figure.tight_layout()
    figure.savefig(MODEL_CHART_PATH, dpi=160, bbox_inches="tight")
    plt.close(figure)
...
def write_report(
    ...
    p_value_text = (
        "< 1e-300" if ttest["p_value"] == 0 else f"{ttest['p_value']:.6e}"
    )
    baseline_mae_reduction = (
        (baseline_metrics["mae"] - metrics["mae"]) / baseline_metrics["mae"] * 100
    )
...
- Pandas 메모리: {load_comparison['pandas_memory_bytes'] / 1024**2:.2f} MiB
- Polars 메모리: {load_comparison['polars_memory_bytes'] / 1024**2:.2f} MiB
- shape·컬럼·결측 건수 일치: Y
...
- 모델 평가 차트: `figures/model_evaluation.png`
...
![Trip distance distribution and correlation heatmap](figures/eda_overview.png)
...
`total_amount`는 연속형 값이므로 분류용 정확도·F1 대신 달러 단위 오차를 보여주는
MAE·RMSE·Median AE와 분산 설명력 R²를 사용했다. 모델 효과를 과장하지 않도록
학습 타깃 평균만 예측하는 `DummyRegressor`를 기준선으로 함께 평가했다.

- 학습: {data['train_rows']:,}행
- 테스트: {data['test_rows']:,}행
- 기준선 MAE: {baseline_metrics['mae']:.4f}
- 기준선 RMSE: {baseline_metrics['rmse']:.4f}
- 기준선 R²: {baseline_metrics['r2']:.4f}
- MAE: {metrics['mae']:.4f}
- RMSE: {metrics['rmse']:.4f}
- Median AE: {metrics['median_ae']:.4f}
- R²: {metrics['r2']:.4f}
- 기준선 대비 MAE 감소: {baseline_mae_reduction:.2f}%
...
![Actual vs predicted, residuals, and fare-band MAE](figures/model_evaluation.png)
...
```json
{json.dumps(metrics_payload['environment'], ensure_ascii=False, indent=2)}
```
...
def main() -> None:
    ...
    print(
        "memory_mib: "
        f"pandas={load_comparison['pandas_memory_bytes'] / 1024**2:.2f}, "
        f"polars={load_comparison['polars_memory_bytes'] / 1024**2:.2f}"
    )
    ...
    baseline.fit(x_train[["trip_distance"]], y_train)
    baseline_predictions = baseline.predict(x_test[["trip_distance"]])
    baseline_metrics = evaluate_regression(y_test, baseline_predictions)
    print(
        "기준선: "
        f"MAE={baseline_metrics['mae']:.4f}, "
        f"RMSE={baseline_metrics['rmse']:.4f}, R²={baseline_metrics['r2']:.4f}"
    )
    ...
    create_model_evaluation_chart(y_test, predictions, fare_bands)
    print(
    ...
    metrics_payload["reload_validation"] = {
        "passed": model_reload_verified,
